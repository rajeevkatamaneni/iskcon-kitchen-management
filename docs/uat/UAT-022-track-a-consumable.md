...
- **Stock is never overwritten.** It is the sum of everything that happened to that item — deliveries
  received, gifts donated, meals cooked, corrections made. There is deliberately no "set the stock to
  40" box on an item that already has stock.
- *Amended 2026-09-19.* The one exception is the **first count** when an item is added: **How much is on
  the shelf now**. It is recorded as the item's opening movement, and when it is above zero the form
  also asks **What it would cost to buy today** (UAT-090). Anyone who manages inventory can enter a first
  count on a new item, however large.
...
| 3 | Add **Rice**, location `Main store`, leave **How much is on the shelf now** empty, warn below `20` | No value is asked, because nothing is being added. You land back on the **list**, with a green line confirming Rice was added. It appears in the list: Item, Location, On hand, the level to warn at, Status |
...
| 7 | Look for any field that lets you type a stock quantity directly on an item **already tracked** | **There is none.** Only the add form has **How much is on the shelf now**, for the first count. Anything else is a Blocker — record exactly where |
...
- [ ] There is no way to overwrite the stock of an item already tracked; only the first count, on adding it.
