...
Status: IN PROGRESS — backend behaviour verified 2026-08-10 (52/52 integration tests green across TenantProvisioningIT, PendingAccountClaimIT, RolePermissionsTest, AuthenticationFilterIT); UAT1-D2 & UAT1-D3 (both minor, test-text only) open; a human browser/visual pass still pending.
...
- **UAT1-D2 (Minor) — OPEN, found 2026-08-10:** This story's "What to look out for" says invalid latitude/longitude → `KMS-4002`. It doesn't. Out-of-range coordinates are caught by bean-validation (`@DecimalMin/@DecimalMax` on `ProvisionTenantRequest`) and returned as `KMS-4001` (`VALIDATION_FAILED`) with the per-field message "Latitude must be between -90 and 90." `INVALID_COORDINATES` (`KMS-4002`) is declared in `ErrorCode.java` but thrown nowhere in `src/main`. `TenantProvisioningIT.refusesInvalidCoordinates` corroborates — it asserts the message, not a code. The **user-facing behaviour is correct** (400 + a clear inline field error + a quotable code), so this is a test-text defect, not a product bug. Resolution options: (a) correct this story to expect `KMS-4001` and retire/reserve `4002`; or (b) validate coordinates in `TenantProvisioningService` and throw `INVALID_COORDINATES` to give them their reserved dedicated code. **Recommend (a)** — the per-field message is better UX than one blanket code. Awaiting Rajeev's call.
- **UAT1-D3 (Minor) — OPEN, found 2026-08-10:** The acceptance criterion "duplicate admin email refused (`KMS-4902`)" is not reachable through provisioning. `users` is unique on `(tenant_id, email)` (`users_email_per_tenant`), and provisioning always creates a brand-new tenant, so the first admin's email can never collide at insert time — the `EMAIL_ALREADY_REGISTERED` catch in `insertFirstAdministrator` is effectively dead on this path. `KMS-4902` **is** genuinely exercised by adding a user to an *existing* temple (UAT-5 / `UserManagementIT`, which asserts it). Resolution: move the duplicate-email expectation from UAT-1 to UAT-5. (Forbidding one person from administering two temples — a globally-unique admin email — would be a product change; I'd advise against it, since a devotee running two temples is plausible.) Awaiting Rajeev's call.
