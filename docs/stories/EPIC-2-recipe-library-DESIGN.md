...
**Source (until 2026-09-19):** `github.com/kranthimj23/ikms`, branch
`ikms-rbac-role-based-access-control`, folder `recipe_books/`. **The library no longer loads from
there** — see the note under §1.

---

## 1. What is actually in that repository

> **Amended 2026-09-19 — the catalogue this section describes has been replaced.** The 32 vendored
> books were deleted from `backend/src/main/resources/recipe-library/` and replaced by two books
> generated by `tools/seed/02b-build-catalogue.mjs` from the recipes Rajeev approved one at a time
> that day: **karnataka (42 recipes) and andhra_pradesh (2) — 44 recipes, 454 ingredient lines, 83
> `prep` values, 15 `not_bought` marks**, counted from the files. He stopped the mass import because
> *"it was a BAD idea to mass import that many recipes without vetting each first."*
>
> **The numbers throughout this section, and the counts in §4 to §13 that come from them, describe
> the upstream repository as it was read on 2026-08-21 and are kept as the record of that
> investigation** — the same rule the 2026-09-08 amendment above applied. Two things in them are no
> longer true of what we ship, and both are worth knowing: the curated books carry a `prep` key and a
> `not_bought` key that no vendored book had, and they carry no `scaled` key at all. What the code
> does today, with figures re-measured against the curated files, is documented in
> `LibraryLoader`, `BookParser`, `IngredientCategories` and
> `backend/src/main/resources/recipe-library/README.md`.
