...
**3a. The merged-tree backend check runs the repo-wide guards too, not only the classes the wave touched.** Added 2026-09-13, after wave 4a failed the release gate on one test the work manager's merged check had never loaded. `BaseQuantityIT` (*"no hand-written unit conversion survives anywhere in the source"*) caught a SQL `CASE` in T-179's `MyDonationsService` that copied `Unit.java`'s labels by hand and got two of them wrong. The merged backend runs named selected classes, as they should for memory, and a guard that scans the whole source belongs to no wave's classes, so it ran first at the gate. It is lesson 3's frontend hole in the backend: a targeted run never loads a repo-wide test. **So every merged-tree backend check now also runs, whatever the wave touched:** the source scans `BaseQuantityIT`, `UnitLabelAgreementTest`, `TempleClockTest` and `CommunicationSendGuardSourceTest`; the whole-list checks `ErrorCodeTest`, `FieldErrorMessageTest`, `NextStepPermissionTest` and `RolePermissionsTest`; the schema checks `RowLevelSecurityIT` and `TenantLoopMigrationIT`; and, when any template or notification code moved, `MetaTemplateRulesTest` and `NotificationTemplateTest`. On the frontend `design-system.test.ts` plays this part and already runs in the full vitest run. **When a new test scans the whole source, schema or a central enum, add it to this list in the same commit.** Found by listing the backend tests that walk `src/main`, read `pg_class` or loop over `values()`; `StockMovementLedgerIT` and `AccessControlEnforcementIT` matched that search and are not repo-wide.

*And the one that search missed, added 2026-09-19 after wave CR-1 failed the merged run on it.* **`PermissionBeforeValidationIT` is repo-wide and belongs in the list above.** T-402 added one endpoint — `PATCH /api/v1/ingredients/{id}/not-bought` — and the whole 3594-test suite came back `1 failed` on *"every write endpoint of the procurement controllers is in the list above"*. The builder could not have caught it: it ran its own classes, as it should, and this class asserts against **the application's own request mappings**, so it belongs to no package and no wave.

The reason the 2026-09-13 search did not find it is worth keeping, because it is the same mistake in the method rather than in the code. That search looked for tests that walk `src/main`, read `pg_class` or loop over `values()` — three ways of enumerating **source, schema or an enum**. This one enumerates neither: it asks Spring at runtime for its `RequestMappingHandlerMapping` and compares that against a hand-written list. **A guard can be repo-wide by reading the running application, not only by reading the tree**, and nothing in the original search would ever have turned one up.

So the rule generalises past this class: **a test is repo-wide if it asserts over a set it discovers for itself, however it discovers it** — the filesystem, the catalog, an enum's `values()`, or the framework's own registry. When you add a test like that, add it here in the same commit. And when a task adds an **endpoint**, expect this guard the way a task that adds a migration expects Flyway: it is the one that notices.
