...
- 2026-09-14 — Phase A fixes released: T-213, T-214, T-215 in commit 6dd436c, CI green, no migration, api-00167-6xb / web-00155-9h6. Record in docs/work/proof/RELEASE-phase-A-fixes.md. Phase B next.
- 2026-09-14 — Phase A fixes T-213..T-215 released: 6dd436c (+8d15ace record), CI green, api-00167-6xb / web-00155-9h6, no migration. Browser retest dispatched -> phase-A-fixes-browser.md. Phase B dispatch started (wave 1), plus a fix for the crew-count 500 without Ready by. PHASE A DONE pending retest.
- 2026-09-14 — PHASE A DONE: retest of T-213..T-215 passed on staging, regression (save+shift, Done/Update, cancel) passed. Proof phase-A-fixes-browser.md. Phase B building.
- 2026-09-14 — Phase B built: T-200..T-212 + T-216 proven, combined run green (frontend 1,900). V141/V142, KMS-400154/400155, package.json changed. Release dispatched (backup first). Open for main session on staging: Meta token accepted for PDF upload?
- 2026-09-14 — Phase B released: 69da777, CI green, backup 1789391583379, api-00168-zjc / web-00156-25b, V141–V142 applied (now v142). Record in docs/work/proof/RELEASE-phase-B.md. Browser test of Phase B next; Meta upload token still unchecked.
- 2026-09-14 — Phase B released: 69da777 (+e738b00 record), CI green, backup 1789391583379, api-00168-zjc / web-00156-25b, V141–V142. Browser test of the 13 items dispatched -> phase-B-browser.md; Meta PDF upload check dispatched (API only) -> phase-B-meta-upload.md. Webhook step put to Rajeev.
- 2026-09-14 — Meta PDF upload check blocked: no App ID stored on staging (a past session used app 1920408082251143, unconfirmed). Asked Rajeev to enter it with the webhook step. Token acceptance still unknown. po_delivery is APPROVED without a header today; adding the header sends it back to Meta review.
- 2026-09-14 — Phase B browser test: all 13 + T-216 pass (proof phase-B-browser.md). Not provable on staging: item 1 end-to-end (waits on App ID), item 8 variance (no bill has one), item 10 old-card fingerprint. Cleanup dispatched: TRACEABILITY.md line 203 stale, UAT-test wish showing on public Give page.
- 2026-09-14 — TRACEABILITY.md stale line fixed. UAT-test wish archive (DELETE /api/v1/wishlist/2a9efaa9-…) blocked by the permission check; left for Rajeev. Docs-only commit dispatched. Waiting on Rajeev: App ID + webhook.
- 2026-09-14 — Test records committed 00dac8f, CI green. Build complete except item 1 end-to-end (PDF send + webhook check), waiting on Rajeev: App ID, webhook, and archive of UAT-test wish.
- 2026-09-14 — App ID + webhook done by Rajeev (verified, app unpublished). Resumable upload with stored token ACCEPTED; po_delivery PENDING with DOCUMENT header since 20:32 UTC. No sample webhook seen yet. Agent polling for approval, then one send to Rajeev + status-webhook check.
