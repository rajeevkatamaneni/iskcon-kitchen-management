...
**Status:** LOCKED — v1.1, amended 2026-09-10. Stage 3 complete.
**Version:** 1.1
**Approved:** 1.0 on 2026-08-03 by Rajeev; v1.1 on 2026-09-10 by Rajeev
**Last updated:** 2026-09-10 (§5 records that recurring donations moved to Phase 2; the Razorpay decision stands and its rows are unedited — see CHANGELOG.md)
...
> **Amended 2026-09-10 — the requirement this section was decided on is now Phase 2, and the
> decision stands.** Recurring donations left Phase 1 entirely on Rajeev's ruling of 2026-09-10 and
> now sit in the Phase 2 Backlog (REQUIREMENTS.md §4). Release 1 takes one-time gifts only: it
> registers no mandate, creates no subscription, and handles no cycle charge.
>
> **Razorpay remains the pick and the evaluation below is not reopened.** Recurring is still coming,
> so the need the choice was made against has moved in time rather than disappeared, and re-running a
> provider comparison for a feature nobody is building this release would be churn. What changes is
> only what the choice is load-bearing for *today*: **read the two rows below as the Stage 3 record
> of a 2026-08-03 evaluation, not as a live Phase 1 dependency.** Nothing in release 1 rests on UPI
> Autopay or eNACH, and no one should read the Razorpay row as a reason release 1 cannot ship on
> another gateway — the Phase 1 half of the case (provider-hosted UPI-first checkout, signed webhooks,
> documentation quality, adoption among Indian nonprofits) stands on its own, and the recurring half
> is now a bet on Phase 2.
>
> **The rows are left exactly as written, deliberately.** They record what was evaluated and decided
> on 2026-08-03, when donor-chosen-frequency recurring giving genuinely was a Phase 1 requirement.
> Editing them to match a later decision would falsify the evaluation rather than update it — the
> Cashfree row in particular, whose narrow rejection turned on that requirement, and whose own advice
> to re-quote both at implementation time is now advice for whoever starts the Phase 2 engagement.
