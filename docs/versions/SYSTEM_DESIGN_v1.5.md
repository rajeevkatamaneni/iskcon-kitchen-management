...
**Status:** LOCKED — v1.5, amended 2026-09-10. Stage 2 complete.
**Version:** 1.5
**Approved:** 2026-08-03 by Rajeev (v1.1 §10 revision approved 2026-08-11; v1.2 on 2026-08-30; v1.3 on 2026-08-31; v1.4 on 2026-09-08; v1.5 on 2026-09-10)
**Last updated:** 2026-09-10 (`recurring_plans` is off the entity list and the payment section stops promising mandates — recurring donations are Phase 2, see CHANGELOG.md)
...
```
        ...
        │  │ calendar precompute, shopping-list generation         │  │
        │  └───────────────────────────────────────────────────────┘  │
        └───────┬───────────────┬──────────────────┬─────────────────┘
                │               │                  │
     ┌──────────▼─────┐ ┌───────▼────────┐ ┌───────▼────────────────┐
...
- **Background jobs: Postgres-backed queue** (e.g. SKIP LOCKED pattern via an established library — Stage 3 pick). Used for: scheduled shift reminders, one-off broadcasts, WhatsApp/SMS/email sends, PDF generation, translation calls, nightly calendar precompute, auto shopping-list generation.
...
- **Audit log:** append-only `audit_events` table (actor, tenant, action, entity, before/after, timestamp, reason). Written by the shared kernel for: financial records, inventory adjustments, calendar date overrides, role changes. No deletes; retention indefinite at pilot scale.
...
- **Key entity groups** (full ERD in Stage 4 stories): tenants/users/roles; ingredients/recipes/recipe_items; kitchens (the several a temple runs, flat under the tenant — v1.2); inventory_items/stock_movements (every stock change is a movement row — receipts, consumption, in-kind donations, adjustments, and ingredients issued to another of the temple's kitchens — giving audit-friendly inventory by construction); ingredient_requests/lines/dishes (a kitchen asking the store, and what it was given — v1.2); meal_plans/planned_meals; vendors/purchase_orders/po_lines/receipts/invoices; shifts/signups/waitlist_entries/reminder_configs; donations/donors/wishlist_items; calendar_days (precomputed per tenant-year, with override rows).
  - *Amended 2026-09-10:* **`recurring_plans`** was the fourth table in the donations group and is
    gone from it. Recurring donations left Phase 1 entirely on Rajeev's ruling of 2026-09-10 and sit
    in the Phase 2 Backlog (REQUIREMENTS.md §4); the table, the rows in it and the plan endpoints
    were dropped with the feature. This line is a live description of the schema, so it is amended
    rather than annotated — a reader designing against it would otherwise look for a table that is
    not there. Nothing else in the donations group changes: `donations` still carries every one-time
    gift, wish-list sponsorship and in-kind intake. The name is left free for Phase 2 to take back.
...
- **Payment gateway (UPI-first):** integration via provider-hosted checkout (never touch raw card/UPI credentials — keeps us out of PCI scope). Webhooks with signature verification + idempotent processing record payment events. **Release 1 takes one-time payments only.** Recurring donations moved to the Phase 2 Backlog on
2026-09-10 (REQUIREMENTS.md §4), and the gateway's mandate/subscription primitives (UPI Autopay/
e-mandate) went with them — nothing in Phase 1 registers a mandate, and no subscription-cycle event
reaches the webhook handler, which serves one-time payment capture and wish-list checkout. Those
primitives are what Phase 2 will build on when it takes the feature up, and they are part of why the
gateway was chosen; that choice is not reopened. Provider pick is Stage 3.
...
- Heaviest operations are all **background jobs** (PDF render, translation, reminder fan-out, shopping-list generation) — precisely why they're off the request path.
