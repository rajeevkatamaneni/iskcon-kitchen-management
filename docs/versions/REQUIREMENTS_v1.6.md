...
**Status:** LOCKED — v1.6, amended 2026-09-10.
**Version:** 1.6
**Approved:** 1.0 on 2026-08-03 by Rajeev; v1.1 on 2026-08-20 by Rajeev; v1.2 on 2026-08-30 by Rajeev; v1.3 on 2026-08-31 by Rajeev; v1.4 on 2026-09-04 by Rajeev; v1.5 on 2026-09-08 by Rajeev; v1.6 on 2026-09-10 by Rajeev
**Last updated:** 2026-09-10 (recurring donations withdrawn from Phase 1 and moved to the Phase 2 Backlog; Phase 1 giving is one-time only — see CHANGELOG.md)
...
- Two inventory classes:
  ...
  - **Equipment** — non-perishable kitchen assets. Tracked by condition, location, and service status rather than depletion. Service status means a recorded servicing history, a service interval, the resulting next-service date, and who services it — with equipment past its date surfaced to the temple administrator.
...
- Plan meals by date. The **three main meals** — breakfast, lunch and dinner — are cooked daily for the temple's own household of staff, volunteers, brahmacharis and residents. **Everything else is an event**, planned as its own preparation with its own name, quantified by how much to make rather than by a head count, and either kept in-house or sent outside for pickup or delivery.
- An outside event captures who is receiving it and how to reach them, and a delivered one captures where it is going and when the guests eat — from which the temple is told when to leave, using a traffic-aware travel estimate.
- Regular, weekend and festival remain the day's character, drawn from the calendar; they are not a classification of the meal.
- Each planned meal references a recipe and an estimated serving count.
- Displays ingredient sufficiency status against current inventory; shortfalls feed the Ordering module's auto-generated shopping list.
...
- **Auto-generated shopping list** — computed from current inventory levels against upcoming meal plan requirements over a configurable horizon. Staff review, adjust, and approve before purchase orders are issued.
...
- **Donations** — one-time gifts, given by a signed-in devotee under their own name.
- **Recurring donations — Decided 2026-09-10: defer to Phase 2.** Donor-selected frequency and the
  standing mandate underneath it leave Phase 1 entirely. Release 1 takes one-time gifts only; a
  devotee who wants to give every month gives every month, one gift at a time, and nothing in the
  product registers a mandate or waits on a subscription-cycle webhook. Recurring giving is not a
  form with a frequency dropdown on it — it is a mandate product whose consequences arrive for
  months after the donor has closed the page (UPI Autopay or eNACH registration, cycle charges,
  failed-cycle retries, pause and cancellation that must reach the gateway as well as our own
  record), and half of it built in a hurry is worse than none of it. Rajeev, 2026-09-10: *"needs to
  be researched properly and built. we will do it later as an engagement once the app is live."* The
  Phase 2 Backlog entry in Section 4 carries it; the gateway chosen partly for it (Razorpay,
  TECH_STACK.md §5) is not reopened.
...
| Recurring donations | Donor-selected frequency (weekly / monthly / quarterly / annually) on a standing mandate — UPI Autopay or eNACH — with cycle charges recorded against a plan, failed-cycle handling, and a donor's own screen to see the next charge and stop it. **Moved out of Phase 1 on 2026-09-10**, having been Phase 1 scope until then: the recurring half is a mandate product with months of webhook consequences after the donor has left the screen, and it needs researching and building as its own engagement once the application is live rather than being carried half-finished through release 1. One-time giving is unaffected and stays in Phase 1. The gateway choice made partly on this requirement stands and is not reopened. |
