package org.iskcon.kms.calendar.astro;

/**
 * Faithful port of GCAyanamsha.py.
 *
 * <p>Precession of the equinoxes. The default ayanamsha type is 1 (Lahiri).
 */
public final class GcAyanamsha {

    private GcAyanamsha() {
    }

    private static int ayamashaType = 1;

    public static double getAyanamsa(double jdate) {
        double t;
        double d;
        double a1 = 0.0;
        // progress of ayanamsa from 1950 to 2000
        t = (jdate - 2451545.0) / 36525.0;
        d = (5028.796195 - 1.1054348 * t) * t / 3600.0;
        if (ayamashaType == 0) { // Fagan-Bradley
            a1 = 24.8361111111 - 0.095268987143399 + d;
        } else if (ayamashaType == 1) { // Lahiri
            a1 = 23.85305555555 + d;
        } else if (ayamashaType == 2) { // Krishnamurti
            a1 = 23.8561111111 - 0.095268987143399 + d;
        } else if (ayamashaType == 3) { // Raman
            a1 = 22.5066666666 - 0.095268987143399 + d;
        }
        return a1;
    }

    public static String getAyanamsaName(int nType) {
        String[] pNames = {
            "Fagan/Bradley",
            "Lahiri",
            "Krishnamurti",
            "Raman"
        };
        if (nType > 3) {
            return "";
        }
        return pNames[nType];
    }

    public static int getAyanamsaType() {
        return ayamashaType;
    }

    public static int setAyanamsaType(int i) {
        int prev = ayamashaType;
        ayamashaType = i;
        return prev;
    }

    public static double getLahiriAyanamsa(double d) {
        double[] h = {
            2415259.000000, 22.475,
            2433486.000000, 23.16472,
            2451791.000000, 23.870277778,
            2455437.000000, 24.001111111,
            2469979.000000, 24.558055556,
            2488234.000000, 25.258055556
        };

        if (d > h[10]) {
            return (d - h[10]) * ((h[11] - h[9]) / (h[10] - h[8])) + h[11];
        } else if (d > h[8]) {
            return (d - h[8]) * ((h[11] - h[9]) / (h[10] - h[8])) + h[9];
        } else if (d > h[6]) {
            return (d - h[6]) * ((h[9] - h[7]) / (h[8] - h[6])) + h[7];
        } else if (d > h[4]) {
            return (d - h[4]) * ((h[7] - h[5]) / (h[6] - h[4])) + h[5];
        } else if (d > h[2]) {
            return (d - h[2]) * ((h[5] - h[3]) / (h[4] - h[2])) + h[3];
        } else if (d > h[0]) {
            return (d - h[0]) * ((h[3] - h[1]) / (h[2] - h[0])) + h[1];
        } else {
            return (d - h[0]) * ((h[3] - h[1]) / (h[2] - h[0])) + h[1];
        }
    }
}
