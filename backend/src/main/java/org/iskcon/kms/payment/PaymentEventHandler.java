 ...
 * beans — captured payments (E7-S2/S6) — and the webhook service dispatches each stored event to
 * whichever handlers claim its type. A handler must be idempotent: the same event may be dispatched
 * again on replay of a dead letter.
 */
public interface PaymentEventHandler {
	 ...
	 * broader than what we ask for — a handler may claim a whole prefix and quietly ignore the events
	 * in it that it has no use for — but a subscription list must be exact, because it is typed into
	 * a provider's dashboard or sent to its API, and neither understands "everything starting with".
	 * Every screen and every registration reads this, so the set a temple subscribes to cannot drift
	 * from the set this application acts on.
	 */
